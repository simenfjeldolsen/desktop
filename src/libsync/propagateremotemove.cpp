...
void PropagateRemoteMove::start()
{
    ...
    if (!_item->_encryptedFileName.isEmpty()) {
        ...
        auto job = new LsColJob(propagator()->account(), root_dir, this);
        ...
        connect(job, &LsColJob::directoryListingSubfolders, this, [=](const QStringList &items) {
            ...
            _fileId = folderInfo.fileId;

            auto lockJob = new LockEncryptFolderApiJob(propagator()->account(), folderInfo.fileId, this);
            connect(lockJob, &LockEncryptFolderApiJob::success, this, [=] (const QByteArray& fileId, const QByteArray& token) {
                ...
                _lockToken = token;
                auto job = new GetMetadataApiJob(propagator()->account(), _fileId);
                connect(job, &GetMetadataApiJob::jsonReceived, this, [=] (const QJsonDocument &json, int statusCode) {
                    ...
                    FolderMetadata* folderMetadata = new FolderMetadata(propagator()->account(), json.toJson(QJsonDocument::Compact), statusCode);

                    QFileInfo info(propagator()->fullLocalPath(_item->_encryptedFileName));
                    ...
                    if (!_fileId.isEmpty() && !_lockToken.isEmpty()) {
                        auto updateMetaDataJob = new UpdateMetadataApiJob(propagator()->account(),
                                                          _fileId,
       ...
       /* auto job = new OCC::UnSetEncryptionFlagApiJob(propagator()->account(), _item->_fileId, this);
        ...
        auto job = new LsColJob(propagator()->account(), remoteSource, this);
        job->setProperties({"resourcetype", "http://owncloud.org/ns:fileid"});
        connect(job, &LsColJob::directoryListingSubfolders, this, [=](const QStringList &items) {
            ...
            connect(lockJob, &LockEncryptFolderApiJob::success, this, [=] (const QByteArray& fileId, const QByteArray& token) {
                Q_UNUSED(fileId) // Should we skip file deletion in case of failure?
                _lockToken = token;
                auto job = new GetMetadataApiJob(propagator()->account(), _item->_fileId);
                connect(job, &GetMetadataApiJob::jsonReceived, this, [=] (const QJsonDocument &json, int statusCode) {
                    Q_UNUSED(json) // Should we skip file deletion in case of failure?
                    Q_UNUSED(statusCode) // Should we skip file deletion in case of failure?
                    Q_UNUSED(fileId) // Should we skip file deletion in case of failure?
                    ...
                    QFileInfo info(propagator()->fullLocalPath(_item->_file));
                    const QString fileName = info.fileName();
                    ...
                    if (!_fileId.isEmpty() && !_lockToken.isEmpty()) {
                        ...
                        connect(updateMetaDataJob, &UpdateMetadataApiJob::error, this, [=](const QByteArray& fileId, int httpReturnCode) {
                            ...
                            if (!_lockToken.isEmpty()) {
                                auto unlockJob = new UnlockEncryptFolderApiJob(propagator()->account(),
                                                                               folderInfo.fileId, _lockToken, this);
